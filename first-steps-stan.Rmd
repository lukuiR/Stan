...
Maud is 73. She likes cats and gardening.

She also has a gambling problem.
</aside>

## Pay Table: Cat and Mouse {.center}
...
<aside class="notes">
Her weakness is slot machines. She's positively obsessed by them. And she has one particular favourite called "Cat and Mouse". The only information that's publicly available about this game is the paytable.
</aside>
...
<aside class="notes">
But Maud wants to know more. Specifically:
...
<aside class="notes">
Somewhat surprisingly, Maud is also a big fan of R. So we embarked on learning some Bayesian techniques together.
</aside>
...
<aside class="notes">
In the context of a slot game, for each session we are considering the number of spins (or "trials") denoted by $n$ and the number of hits (or "successes") denoted by $k$. The probability of success on any spin is $\theta$.
...
<aside class="notes">
However we'd be multiplying together a load of small numbers, which would mean that we would rapidly lose precision. Instead we work with the log likelihood.
...
<aside class="notes">
Bayesian Theorem is probably familiar to many of you, but let's just take a moment to review.

The Bayesian technique is not entirely dissimilar to Maximum Likelihood, however, it introduces the concept of the prior, which is multiplied by the likelihood to give the posterior. Basically we are saying, "We have some existing knowledge about the model parameter (given by the prior) and we know how the likelihood of the model parameters are affected by the data (given by the likelihood). And we use these pieces of information to get the probability of the "

A Bayesian model is the combination of a prior and a likelihood.

The prior is specifies what we know about the model parameters before considering the data.
...
<aside class="notes">
Let's take a look at how Bayes' Theorem could be applied. We'll consider the Binomial Distribution and aim to quantify the successrate. Starting with a uniform prior (we have no idea what the success rate is to start with), we repeatedly apply Bayes' Theorem. Each iteration ingests more data and the posterior becomes progressively narrower, presumably centred on the true value.
...
Although simple Monte Carlo is better in higher dimensions than the grid approximation, it's still far from perfect.
...
<aside class="notes">
As an experienced gambler Maud knew that the likelihood of the hit rate was not uniformly distributed between 0 and 1. We could have factored this knowledge into the computation by applying a more informed prior. For example, a broad beta distribution peaked at 0.5. The results are essentially unchanged though.
...
## {data-background="img/maud-not-cricket.svg" data-background-size="50%" data-background-position="left bottom"}
...
<aside class="notes">
Finally here's the kicker. Since we have access to the posterior distribution we are able to calculate all manner of statistics without relying on (poor) asymptotic approximations. For example, we can find the probability of breaking even or doubling our money.

These figures are enough to make Maud reconsider her gambling fixation.
</aside>

## {data-background="img/maud-crochet.svg" data-background-size="50%" data-background-position="left bottom"}
