...
<aside class="notes">
Somewhat surprisingly, Maud is also a big fan of R. So the two of us formed a litte study group to learn about Bayesian techniques.
</aside>
...
<aside class="notes">
Fortunately Maud has diligently recorded her gambling statistics. As a result we have a rather nice, rich data set to play with. At the session level the most important bits of information are the number of spins and the associated number of hits.
</aside>

## Maud's Data (by Spin)

```{r}
(spin <- sessions %>% select(details) %>% unnest() %>% mutate(success = as.integer(payout > 0)))
```
...
## Why Stan? (and Bayesian techniques in general) {.center}
...
<div style="text-align: center; margin-top: 50px; font-size: 3em; color: DodgerBlue;">Do cool things with Stan!</div>
