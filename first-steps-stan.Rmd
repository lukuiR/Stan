...
```{r echo=FALSE}
(rtp_summary = summary(fit, probs = c(0.025, 0.975), pars = c(paste0("theta[", 1:6, "]"), "sigma", "pay"))$summary)
```

Overall RTP is `r format(rtp_summary[8,1] * 100, digits = 3)`.

The 1x payout is triggered once in every `r format(1 / rtp_summary[1,1], digits = 3)` spins.
...
```{r hist_rtp_simulated, echo=FALSE, cache=TRUE}
...
<aside class="notes">
So, returning to Maud's three burning questions, not only have we managed to obtain point estimates to answer each of them, but we have actually generated distributions for each of these quantities, which are enormously more informative.
</aside>
